...
Ok not too many NAs execpt for the `Notes` column, we might want to discard this column before importing this table in our databse. Also glimpse hightlighted that the data types are not what we were expecting for the percentages of cover. Let's find out why:
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>%
  mutate(Date = ifelse(Date == "8&9 june 06", "8 june 06", Date),   # Arbitrary
         Date2 = dmy(Date))
```
...
snowsurvey_final <- snowsurvey_fixed %>%
  select(Site, Year, Date2, Plot, Location, Snow_cover, Water_cover, Land_cover, Observer) %>% # Select columns of interest
  rename(Date = Date2) %>%
